...
function Autobus() {
  ...
  function handleClick(index) {
    ...
    if (noviNiz[index]) {
      ...
      setTrenutnaRezervacija(trenutnaRezervacija.filter(s => s !== index + 1));
    }
  ...
  return (
    <div>
      <select value={odabraniAutobus !== null ? odabraniAutobus : ""} onChange={handleSelect}>
      ...
      {odabraniAutobus !== undefined && (
        <div className="autobus">
          ...
          <div>Trenutno rezervisana mesta: {trenutnaRezervacija.length > 0 ? trenutnaRezervacija.join(', ') : 'Nijedno'}</div>
          <button onClick={handlePotvrdi} disabled={trenutnaRezervacija.length === 0}>Potvrdi izbor</button>
        </div>
      )}
    </div>
  );
